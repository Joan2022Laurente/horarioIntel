...
export const DirectConnectModal: React.FC<DirectConnectModalProps> = ({
  ...
  const defaultWhatsappMessage = encodeURIComponent(
    `¡Hola ${targetName.split(' ')[0]}! Te vi en UTP Intel con ventana libre para ${targetCourse}. Estoy en ${currentUserLocation} con la meta de: "${targetGoal}". ¿Nos juntamos a estudiar?`
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150">
      <div 
        ...
        className="relative w-full max-w-lg rounded-3xl bg-[var(--surface-card)] border border-[var(--border-medium)] p-6 space-y-6 shadow-2xl animate-in zoom-in-95 duration-150 text-white"
      >
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="inline-flex items-center gap-0.5 text-[10px] font-bold text-[var(--badge-emerald-text)] bg-[var(--badge-emerald-bg)] border border-[var(--badge-emerald-border)] px-2 py-0.5 rounded-full">
        ...
        {/* Ficha Resumen de la Conexión */}
        <div className="rounded-2xl bg-[var(--surface-subtle)] border border-[var(--border-subtle)] p-4 space-y-3 text-xs">
          {buddy && (
          ...
          <div className="flex items-center justify-between">
            <span className="text-neutral-400">Asignatura en Común:</span>
            <span className="font-bold text-white text-right">{targetCourse}</span>
          </div>
          ...
          {buddy?.sharedWindow && (
            <div className="flex items-center justify-between">
              <span className="text-neutral-400">Ventana Libre en Común:</span>
          ...
          <div className="pt-2 border-t border-[var(--border-subtle)]">
            ...
            <p className="text-white font-medium bg-[var(--surface-input)] p-2.5 rounded-xl border border-[var(--border-subtle)]">
              &ldquo;{targetGoal}&rdquo;
            </p>
          </div>
        </div>

        {/* Canales de Contacto Directo ("Handshake") */}
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 p-3 rounded-2xl bg-[var(--surface-muted)] hover:bg-[var(--surface-elevated)] border border-[var(--border-medium)] text-white font-bold text-xs transition active:scale-95 shadow-none"
            >
              <Video className="h-4 w-4 text-[var(--accent-blue)]" />
              <span>Crear Sala Meet / Discord</span>
            </a>
          </div>
        </div>
        ...
        <div className="space-y-2 pt-2 border-t border-[var(--border-subtle)]">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-[var(--surface-input)] border border-[var(--border-subtle)] rounded-xl px-3.5 py-2 text-xs text-white focus:outline-none focus:border-[var(--accent-emerald)]"
            />
            <button
