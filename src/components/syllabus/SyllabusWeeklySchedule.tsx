...
export const SyllabusWeeklySchedule: React.FC<SyllabusWeeklyScheduleProps> = ({
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="max-h-64 overflow-y-auto rounded-2xl bg-[#181820] border border-white/10 p-2 space-y-1.5 custom-scrollbar">
        {schedule.map((s, idx) => {
          ...
          return (
            <div key={idx} className="p-3.5 rounded-xl flex items-start justify-between gap-3 text-xs bg-[#101014] hover:bg-[#20202c] border border-white/10 transition">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="font-mono font-bold text-[var(--accent-lime)]">
                    Semana {s.week} {s.session ? `• Sesión ${s.session}` : ''}
                  </span>
                  <span className="text-[10px] font-mono bg-[#181820] border border-white/10 text-neutral-300 px-2 py-0.5 rounded-full">
                    {s.unit}
                  </span>
                  {s.evaluation && (
                    <span className="text-[10px] font-black uppercase bg-[var(--accent-orange)]/15 border border-[var(--accent-orange)]/30 text-[var(--accent-orange)] px-2 py-0.5 rounded-full">
                      Entrega {s.evaluation}
                    </span>
                  )}
                  {s.isDeliverableForClassScore && (
                    <span className="text-[10px] font-bold bg-[#181820] border border-white/10 text-neutral-300 px-2 py-0.5 rounded-full">
                      Nota PA
                    </span>
                  )}
                </div>
