      ...
      <div className="space-y-2.5">
        {sortedEvaluations.map((ev) => {
          ...
          return (
            <div 
              key={ev.id} 
              className="p-4 sm:p-5 rounded-2xl bg-[var(--surface-card)] hover:bg-[var(--surface-card-hover)] border border-[var(--border-subtle)] hover:border-[var(--border-medium)] flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-colors shadow-none"
            >
              <div className="space-y-1">
                <div className="flex flex-wrap items-center gap-2 text-xs">
                  ...
                  <span className={`font-semibold ${isThisWeek ? 'text-[var(--accent-lime)]' : isNextWeek ? 'text-[var(--accent-orange)]' : 'text-neutral-400'}`}>
                    Semana {ev.week} {isThisWeek ? '(Esta semana)' : ''}
                  </span>
                  <span className="text-neutral-600">•</span>
                  <span className="font-bold text-[var(--badge-orange-text)] bg-[var(--badge-orange-bg)] border border-[var(--badge-orange-border)] px-2 py-0.5 rounded-md text-[11px]">
              ...
              <div className="flex items-center gap-2 shrink-0 pt-2 sm:pt-0">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-xl bg-[var(--surface-subtle)] hover:bg-[var(--surface-muted)] border border-[var(--border-subtle)] px-3 py-1.5 text-xs font-semibold text-neutral-300 hover:text-white transition shadow-none"
                >
