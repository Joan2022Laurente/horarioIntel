...
export function getCachedSyllabus(courseNameOrCode: string): ParsedSyllabus | null {
  ...
  try {
    ...
    const direct = localStorage.getItem(`${SYLLABUS_STORAGE_PREFIX}${courseNameOrCode}`);
    if (direct) return JSON.parse(direct);

    // 2. Búsqueda en el mapa global de localStorage
  ...
  // 3. Fallback al registro oficial por defecto
  return getSyllabusForCourse(courseNameOrCode);
}
